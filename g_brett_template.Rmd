...
for( i in seq_len( NROW(peaks) ) ){
  ...
  peaks$TOP_POPS[i] <- paste( locus$GENE[pops_idx], collapse="; " )
}

# Print table
gcols_peaks <- c( "NEAREST_GENE", "TOP_POPS", "SNP", "CHR", "BP", "LO", "HI", "P" )
peak_caption <- paste( "A table of GWAS hits. NEAREST_GENE = gene nearest to the",
                       "lead variant, TOP_POPS = gene with the highest POPS in",
                       "the locus, SNP = variant identifier, CHR = chromosome,",
                       "BP = position, LO = leftmost boundary of the peak, HI =",
                       "rightmost boundary of the peak, P = P value" )
...
# Test section
```{r test_section, results="asis"}
for( i in 1 ){
  pops_locus_plot <- file.path( "plots", "pops", paste0( "region_", i, "_rs35749011.jpg" ) )
  cat( paste0( "![](", pops_locus_plot, ")" ), "\n")
}
```

# Locus plots {.tabset .tabset-pills}
```{r locus_plots, echo=FALSE, results="asis"}

# Loop through peaks
setwd(params$maindir)
for( i in seq_len( NROW(peaks) ) ){
