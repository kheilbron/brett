...
peaks$SNP   <- sub( pattern=pattern, replacement="\\2", x=peaks$HIT )
peaks$LOCUS <- sub( pattern=pattern, replacement="\\1", x=peaks$HIT )
...
peaks$POPS <- peaks$SCORE <- peaks$POPS_Q <- peaks$MAGMA <- peaks$N_GENES <- NA
for( i in seq_along(peaks$HIT) ){
  ...
  # Find the gene with the top POPS, its score, and the number of genes in the locus
  if( NROW(locus) > 0 ){
    ...
    peaks$POPS_Q[i] <- locus$Q_POPS[ pops_idx[1] ]
    peaks$MAGMA[i]  <- locus$Z_MAGMA[ pops_idx[1] ]
  }
  peaks$N_GENES[i] <- NROW(locus)
}

# Add a column denoting whether the nearest gene is also the top POPS gene
# Plus more stringent gene prioritization criteria
peaks$BOTH     <- peaks$NEAREST == peaks$POPS & !is.na(peaks$POPS)
peaks$PRIORITY <- peaks$NEAREST == peaks$POPS & !is.na(peaks$POPS) & 
                  peaks$SCORE    > crit_pops &
                  peaks$N_GENES  <= 12

# Write peaks data to file
fwrite( x=peaks, file=file.path( maindir, "peaks.tsv" ), sep="\t" )

# Print table
gcols_peaks <- c( "SNP", "CHR", "CENTRE", "LO", "HI", "P", "POPS", "SCORE", 
                  "NEAREST", "DIST", "N_GENES", "BOTH", "PRIORITY" )
peak_caption <- paste( "A table of GWAS hits. SNP = variant identifier, CHR =",
                       ...
                       "of the peak, P = top-PIP SNP P value, POPS = gene with",
                       "the highest POPS in the locus, SCORE = POPS score,",
                       "NEAREST = gene nearest to the PIP-weighted position,",
                       "DIST = distance between PIP-weighted position and the",
                       "nearest gene, N_GENES = number of genes in the locus,",
