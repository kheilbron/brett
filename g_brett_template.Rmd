...
# The top 1% of POPS
crit_pops <- pops$POPS[ order( pops$POPS, decreasing=TRUE ) ][ round( NROW(pops)/100 ) ]
...
# Look at fine-mapped loci 
# Nearest gene = top POPS gene and POPS > critical value
fm_peaks <- peaks[ peaks$NEAREST_GENE == peaks$POPS_GENE & peaks$POPS > crit_pops , c(-1,-4,-5) ]
fm_peaks <- fm_peaks[ order( fm_peaks$POPS, decreasing=TRUE ) , ]
fwrite( fm_peaks, file.path( maindir, "fine_mapped_peaks.tsv"), sep="\t" )
...
for( i in seq_len( NROW(peaks) ) ){
  ...
  locus_heading <- paste0( "\n## Locus ", i, ": ", peaks$SNP[i], " {.tabset .tabset-pills}\n")
  locus_subheading <- paste0( "Chr", peaks$CHR[i], " ", round(peaks$BP[i]/1e6), 
                              "Mb, Near: ", peaks$NEAREST_GENE[i], 
                              ", PoPS: ", peaks$POPS_GENE[i], "\n" )
  cat(locus_heading)
